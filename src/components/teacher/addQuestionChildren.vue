      ...
      <el-tab-pane name="first">
        ...
        <section class="append">
          <ul>
            <li>
              <span>题目类型:</span>
              <!-- 选择题型，遍历，将options中的值显示在el-option中，并将选中值与optionValue值进行绑定 -->
            ...
            <li v-if="optionValue === '选择题'">
              <span>难度等级:</span>
              <el-select v-model="postChoice.level" placeholder="选择难度等级" class="w150">
                <el-option
            ...
            <!-- 如果当前选中的题型是选择题，则再添加一个li标签，用于选择本题的正确答案 -->
            <li v-if="optionValue === '选择题'">
              <span>正确选项:</span>
              <el-select v-model="postChoice.rightAnswer" placeholder="选择正确答案" class="w150">
                <el-option
          ...
          <!-- 选择题部分，包括题目内容、四个选项的内容以及题目解析的内容 -->
          <div class="choice" v-if="optionValue === '选择题'">
            <div class="title">
              <el-tag>题目:</el-tag>
          ...
          <!-- 填空题部分。包括题目内容、正确答案以及题目解析 -->
          <div class="choice fill" v-if="optionValue === '填空题'">
            <div class="title">
              <el-tag>题目:</el-tag>
          ...
          <!-- 判断题部分。包括题目内容，正确答案和错误答案、以及答案解析 -->
          <div class="choice judge" v-if="optionValue === '判断题'">
            <div class="title">
              <el-tag>题目:</el-tag>
      ...
      <el-tab-pane name="second">
        ...
        <div class="box">
          <ul>
            ...
            <li>
              <span>选择题数量：</span>
              <el-input type="text" v-model="choiceNumber"></el-input>
            </li>
            <li>
              <span>填空题数量：</span>
            ...
            <li>
              <el-button type="primary" @click="assemble()">立即组卷</el-button>
            </li>
          </ul>
        </div>
      </el-tab-pane>
...
export default {
  ...
  methods: {
    ...
    assemble() {
      ...
      this.$axios({
        ...
        data: {
          ...
          level: this.difficultyValue,  //题目难度
          subject: '计算机网络', //由于数据库中题目太少题目数量太少，默认指定从计算机网络中抽题
    ...
    choiceSubmit() { //选择题题库提交
      console.log('------------------' + this.paperId)
      // ToDo 提交题库改进
      // 原来的逻辑：先将题目提交到数据库中，后端返回添加成功的响应。再次查询数据库，将最新的一条提交记录返回，为的就是获取数据库自增的questionId信息。最后将paperId与questionId提交到数据库中，表示该道题是属于哪张试卷的。一个exam对应一张试卷。提交一道题前后端交互3次，访问数据库3次
      // 改进后的逻辑：由于paperId是已知的，直接将paperId与题目内容进行提交。在后端中，mybatis将题目插入时会将自增的主键id返回，此时再将paperId与questionId插入到数据库中。前后端交互1次，访问数据库2次。
      this.postChoice.subject = this.subject
      this.$axios({ //提交数据到选择题题库表
        url: '/apis/question/addChoice/' + this.paperId,
        method: 'post',
        data: {
          ...this.postChoice
        }
    ...
    fillSubmit() { //填空题提交
      ...
      this.$axios({
        url: '/apis/question/addFill/' + this.paperId,
        method: 'post',
        data: {
          ...this.postFill
        }
    ...
    judgeSubmit() { //判断题提交
      ...
      this.$axios({
        url: '/apis/question/addJudge/' + this.paperId,
        method: 'post',
        data: {
          ...this.postJudge
        }

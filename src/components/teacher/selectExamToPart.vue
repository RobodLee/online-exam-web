<!-- 成绩分段查询 -->
<!-- 首先将所有考试信息查询出来，在页面进行展示，当点击查看分段时，跳转到scorePart组件，并将考试编号和考试名称传入。在scorePart组件中，通过考试编号将该次考试的所有分数查询出来，并将分数进行分段展示 -->
<template>
  <div class="exam">
    <el-table :data="pagination.list" border>
      ...
      <el-table-column fixed="right" label="操作" width="150">
        <template slot-scope="scope">
          <el-button @click="toPart(scope.row.id,scope.row.course)" type="primary" size="small">查看分段</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  methods: {
    getExamInfo() { //分页查询所有试卷信息
      this.$axios(`/apis/exam/${this.pagination.pageNum}/${this.pagination.pageSize}`).then(res => {
    ...
    toPart(id,course) { //跳转到分段charts页面
      console.log(id)
      console.log(course)
      this.$router.push({path: '/scorePart', query:{examCode: id, course: course}})
    }
  },
};
</script>

<!-- 增加题库（试卷展示界面） -->
<!-- 加载所有的试卷信息，点击试卷后面的 增加题库按钮，可以跳转到addQuestionChildren(增加题库)组件，跳转时将paperId(试卷id)以及课程名称传入 -->
<template>
  <div class="exam">
    <el-table :data="pagination.list" border>
      ...
      <el-table-column fixed="right" label="操作" width="150">
        <template slot-scope="scope">
          <el-button @click="addQuestion(scope.row.paperId,scope.row.course)" type="primary" size="small">增加题库</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  methods: {
    ...
    //添加试题
    addQuestion(paperId,course) {
      this.$router.push({
        path: '/addQuestionChildren',
        query: {
          paperId: paperId,
          subject: course
        }

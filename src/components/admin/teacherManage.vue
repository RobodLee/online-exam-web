    ...
    <el-table :data="pagination.list" border>
      ...
      <el-table-column fixed="right" label="操作" width="150">
        <template slot-scope="scope">
          <el-button @click="checkTeacher(scope.row.id)" type="primary" size="small">编辑</el-button>
...
<script>
  export default {
    ...
    methods: {
      getTeacherInfo() {
        //分页查询所有教师信息
        this.$axios(`/apis/teacher/${this.pagination.pageNum}/${this.pagination.pageSize}`).then(res => {
      ...
      checkTeacher(id) { //修改教师信息
        this.dialogVisible = true
        this.$axios(`/apis/teacher/${id}`).then(res => {
          this.form = res.data.data
        })
      },
      deleteById(id) { //删除当前教师
